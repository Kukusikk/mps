...
;-----------------------------------------------------------------------------
BEGIN:
    LCALL LSH_FLAG_REVIEW
LL:	MOV A, #1h					;print Message M_1
	LCALL MESSAGE_PRINT
	LCALL IND_CNT
...
;	JB INT1F, INPUT_LSH_KEY					;wait for key
    MOV SHIFT_PER, #01h						;shift period := 30 (30 * 0.1 sec = 3 sec)
	MOV r7, SHIFT_PER						;mov in programm counter shift period
	SETB ET0								;turn on T0 interruptions
	SETB TR0								;turn on T0 counter
	CLR T0F									;clear my flag
	MM:	JNB T0F, MM				;wait for timer
	LSH_NUM:
	CLR T0F	
	DJNZ r7, MM			;decrease and compare with zero
	LJMP LL
